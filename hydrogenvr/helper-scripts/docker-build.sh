...
then
	...
	#docker run --rm -v $(pwd):/project $TAG_NAME /bin/bash -c "git config --global --add safe.directory /project ; apt install -y qt5-default libqt5multimedia5-plugins qtdeclarative5-dev-tools ; cd /project ; ./hydrogenvr/ci/gitlab-ci/commons/make-appimage.sh ; chmod -R 777 build/"
	docker run --rm -v $(pwd):/project $TAG_NAME /bin/bash -c "git config --global --add safe.directory /project ; cd /project ; ./hydrogenvr/ci/gitlab-ci/commons/make-appimage.sh ; chmod -R 777 build/"
fi

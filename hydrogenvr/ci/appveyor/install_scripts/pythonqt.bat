...
if NOT EXIST pythonqt\ (
	...
	move %APPVEYOR_BUILD_FOLDER%\hydrogenvr\misc\PythonQt.pro .
	move %APPVEYOR_BUILD_FOLDER%\hydrogenvr\misc\python.prf .\build\python.prf
	move %APPVEYOR_BUILD_FOLDER%\hydrogenvr\misc\PythonQt.prf .\build\PythonQt.prf
	move %APPVEYOR_BUILD_FOLDER%\hydrogenvr\misc\PythonQt_QtAll.prf .\build\PythonQt_QtAll.prf
	IF "%BUILD_TYPE%" == "64bit" (
